...
/**
* Sends a queue messages.
* 
* @param {string}             queuePath                                           A string object that represents the name of the queue to which the message will be sent.
* @param {Array<object|string>}      message                                             A message object that represents the message to send.
* @param {string}             [message.body]                                      The message's text.
* @param {object}             [message.customProperties]                          The message's custom properties.
* @param {string}             [message.brokerProperties.CorrelationId]            The message's correlation identifier.
* @param {string}             [message.brokerProperties.SessionId]                The session identifier.
...
* var azure = require('azure');
* var serviceBusService = azure.createServiceBusService();
* serviceBusService.sendQueueMessages('taskqueue', ['Hello world!', 'Hola Mundo!', 'Bonjour le monde!, 'Ciao Mondo!'], function(error) {
*   if(!error) {
*     // Message sent
*   }
* }); 
*/
ServiceBusService.prototype.sendQueueMessages = function (queuePath, messages, callback) {
  validateQueueName(queuePath);
  this._sendMessages(queuePath, messages, callback);
};
...
ServiceBusService.prototype._sendMessage = function (path, message, callback) {
  ...
  var requestOptions = {clientRequestTimeout: 60000}; // 60 seconds.
  this.performRequest(webResource, message.body, requestOptions, processResponseCallback);
...
* @ignore
* @param {string}             path                                                  A string object that represents the path to which the message will be sent. This may be the value of a queuePath or a topicPath.
* @param {List<object|string>}      messages                                               A message object that represents the message to send.
* @param {string}             [message.body]                                        The message's text.
* @param {object}             [message.customProperties]                            The message's custom properties.
* @param {string}             [message.brokerProperties.CorrelationId]              The message's correlation identifier.
* @param {string}             [message.brokerProperties.SessionId]                  The session identifier.
...
* @return {undefined}
*/
ServiceBusService.prototype._sendMessages = function (path, messages, callback) {
  validateCallback(callback);
  
  var _messages = [];

  if (azureutil.objectIsString(messages)) {
    _messages = [{ body: messages }];
  }else if( Object.prototype.toString.call( messages ) === '[object Array]' ) {
    for(var i = 0 ; i < messages.length ; i++){
      var message = messages[i];
       _messages.push(azureutil.objectIsString(message) ? { body: message } : message);
    }
  }
  ...
  setRequestHeaders(webResource, customHeaders);
  ...
  this.performRequest(webResource, JSON.stringify(_messages), null, processResponseCallback);
};
